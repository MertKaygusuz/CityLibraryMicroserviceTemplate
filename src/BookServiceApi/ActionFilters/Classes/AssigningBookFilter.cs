...
{
    public class AssigningBookFilter(IBookService bookService, IUserService userService, IStringLocalizer<ActionFiltersResource> localizer) : IAssigningBookFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            bool isThereAnyAvailableBook = await _bookService.CheckIfAnyAvailableBooksAsync(modelVal.BookId);

            if(isThereAnyAvailableBook is false)
            {
                var err = new ErrorDto();
                err.Errors.Add(nameof(modelVal.BookId), [_localizer["Book_Not_Available"]]);
                context.Result = new ObjectResult(err)
                {
                    StatusCode = err.Status
                };
